...
export default function AuthPage({ mode }: { mode: AuthMode }) {
  ...
  function renderGoogleButton() {
    ...
    window.google.accounts.id.renderButton(googleButtonRef.current, {
      ...
      shape: "pill",
      logo_alignment: "left",
      width: Math.min(560, googleButtonRef.current.clientWidth || 400),
    });
  }
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const emailVal = String(form.get("email") || form.get("contactEmail") || "").trim();

    const payload =
      mode === "signup"
        ? {
            phone: signupMethod === "phone" ? (fullPhone || rawPhone) : "",
            email: signupMethod === "email" ? emailVal : "",
        ...
        : {
            identifier:
              loginMethod === "email"
                ? (emailVal || String(form.get("identifier") || "").trim())
                : (String(form.get("identifier") || "").trim() || fullPhone || rawPhone),
            password: String(form.get("password") || ""),
          };
        ...
        <section className="flex flex-col items-center justify-center px-6 py-10 sm:px-12 lg:px-16 xl:px-20">
          <div className="w-full max-w-[560px]">
            ...
            <form onSubmit={submit} className="mt-8 space-y-5" noValidate>
              {mode === "signup" ? (
                /* -----------------------------------------------------------
                   ...
                   ----------------------------------------------------------- */
                <>
                  {/* Method Switcher: Through Email vs Phone number */}
            ...
            <div className="relative min-h-11 w-full overflow-hidden rounded-full">
              {GOOGLE_CLIENT_ID ? (
                <div ref={googleButtonRef} className={`flex justify-center ${googleLoading ? "pointer-events-none opacity-60" : ""}`} />
              ) : (
                <div className="rounded-full border border-amber-200 bg-amber-50 px-4 py-3 text-center text-sm font-medium text-amber-800">
                  Google sign-in is unavailable until the OAuth Client ID is configured.
                </div>
              )}
              {googleLoading && (
                <div className="absolute inset-0 grid place-items-center bg-white/70">
                  <LoaderCircle className="h-5 w-5 animate-spin text-[#355BFF]" />
                </div>
              )}
            </div>
