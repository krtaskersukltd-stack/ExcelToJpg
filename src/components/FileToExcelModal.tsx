...
import { convertFileToExcel, ExcelSourceKind, triggerFileDownload } from "@/lib/api";
...
export default function FileToExcelModal({ isOpen, onClose, sourceKind }: { isOpen: boolean; onClose: () => void; sourceKind: ExcelSourceKind }) {
  ...
  useEffect(() => {
    if (isOpen) { setFile(null); setStatus("idle"); setOutput(null); setError(""); }
  ...
  const convert = async (nextFile: File) => {
    ...
    setFile(nextFile); setStatus("working"); setError(""); setOutput(null);
    const result = await convertFileToExcel(nextFile, sourceKind, controller.signal);
    if (controller.signal.aborted) return;
    if (result.status === "success") { setOutput(result.filename); setStatus("ready"); }
    else { setError(result.error); setStatus("error"); }
  };

  if (!isOpen) return null;
  const label = `${sourceKind.toUpperCase()} to Excel`;
  return <AnimatePresence><div className="fixed inset-0 z-[80] flex items-center justify-center p-4" role="dialog" aria-modal="true" aria-label={label}>
    <button className="fixed inset-0 bg-slate-950/65 backdrop-blur-md" onClick={onClose} aria-label="Close" />
    <motion.div initial={{ opacity: 0, y: 15, scale: .97 }} animate={{ opacity: 1, y: 0, scale: 1 }} className="relative z-10 w-full max-w-xl overflow-hidden rounded-3xl bg-white shadow-2xl">
      <header className="flex items-center justify-between border-b border-slate-100 bg-blue-50/60 px-6 py-4"><div><h2 className="text-lg font-bold text-slate-950">{label}</h2><p className="text-xs text-slate-500">{sourceKind === "csv" ? "Format CSV rows and columns into an XLSX workbook." : "Extract readable table data into an editable XLSX workbook."}</p></div><button onClick={onClose} className="rounded-full p-2 text-slate-500 hover:bg-white"><X className="h-5 w-5" /></button></header>
      ...
      <div className="p-6">
        ...
        {status === "ready" && <div className="flex min-h-72 flex-col items-center justify-center text-center"><CheckCircle2 className="mb-4 h-14 w-14 text-emerald-500" /><h3 className="text-xl font-bold text-slate-900">Excel workbook ready</h3><p className="mt-1 max-w-md truncate text-sm text-slate-500">{output}</p><button onClick={() => output && void triggerFileDownload(output, output)} className="mt-6 inline-flex items-center gap-2 rounded-xl bg-[#355BFF] px-6 py-3 text-sm font-bold text-white"><Download className="h-4 w-4" />Download XLSX</button><button onClick={() => inputRef.current?.click()} className="mt-3 text-xs font-bold text-blue-700 hover:underline">Convert another file</button></div>}
      </div>
    </motion.div>
  </div></AnimatePresence>;
}
